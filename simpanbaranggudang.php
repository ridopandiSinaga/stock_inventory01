<?php  
include "koneksi.php";
$kdbeli = mysqli_real_escape_string($conn, $_GET['kdbl']);
    if (isset($_POST['save'])) {
		$kodebarang = mysqli_real_escape_string($conn, $_POST['kdbarang']);
		$hargajual = $_POST['hargaj'];
		$ambildatabarang = mysqli_query($conn, "SELECT * FROM barang_pembelian WHERE kd_barang_beli='$kdbeli'");
		$data = mysqli_fetch_assoc($ambildatabarang);
		$namabarang = $data['nama_barang_beli'];
		$satuan = $data['satuan'];
		$hargabeli = $data['harga_beli'];
		$stok = $data['item'];
		
        mysqli_query($conn, "INSERT INTO barang(kd_barang,nama_barang,satuan,harga_jual,harga_beli,stok) 
				VALUES('$kodebarang','$namabarang','$satuan','$hargajual','$hargabeli','$stok')");
		//update data barang pembelian dengan setatus 1
		mysqli_query($conn, "UPDATE barang_pembelian SET status='1' WHERE kd_barang_beli ='$kdbeli'");
        echo "<script>
				bootbox.alert('Data Berhasil Di Tambahkan!', function(){
	...
	$qry = mysqli_query($conn, "SELECT * FROM barang_pembelian WHERE kd_barang_beli = '$kdbeli'");
			...
			<div class="box-body">	
				<form method="POST" id="forminput" enctype="multipart/form-data">
					<div class="form-group">
						...
						<input type="text" style="text-transform:uppercase" class="form-control" name="kdbarang" id="kdbarang" placeholder="Masukan Kode Barang, Ex: BRG00001" maxlength="20">
						<div id="showresult" style="padding-top:4px; padding-bottom:0;"></div>
					</div>
					<div class="form-group">
						<label>Harga Beli</label>
						<input type="text" class="form-control" id="hargaj" readonly="true" value="<?php echo $pecah['harga_beli']; ?>">
					</div>
					<div class="form-group">
